...
export default function ImageUploader() {
  ...
  const uploadToBackend = async (file: File) => {
    setOriginalImage(URL.createObjectURL(file)) // show uploaded image
    ...
    <>
      ...
      {originalImage && transformedImage && (
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 mt-8">
          <div>
            <p className="text-center mb-2 text-sm text-neutral-500">Original</p>
            <img src={originalImage} alt="Original" className="max-w-xs rounded-lg shadow-md" />
          </div>
          <div>
            <p className="text-center mb-2 text-sm text-neutral-500">Transformed (Ghibli Style)</p>
            <img src={transformedImage} alt="Transformed" className="max-w-xs rounded-lg shadow-md" />
          </div>
        </div>
      )}
    </>
